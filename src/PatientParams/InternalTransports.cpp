...
String UartTransport::receive(unsigned long timeout_ms) {
    String response = "";
    unsigned long start = millis();
    
    // Clear buffer first? Maybe not needed if we assume protocol flow.
    ...
    while (millis() - start < timeout_ms) {
        ...
        // Small yield not strictly necessary on Arduino loop, but good practice
        delay(1); 
    }
